// Shared bulk-import logic — the exact findOrCreate/key-resolution upsert
// engine originally built for POST /api/admin/import, extracted so it can
// also be called in-process from a Server Action (see actions.ts /
// importPayload). Both callers wrap this in one BEGIN/COMMIT/ROLLBACK
// around a single pooled client; this function itself does no transaction
// control, so it's safe to call from either.
//
// Records may reference each other via an arbitrary string "key" set on a
// record and referenced by "<field>Key" elsewhere in the same payload.
// Existing rows are matched by natural key (country + lower(name), etc.)
// and reused rather than duplicated, so a payload can be safely re-posted.
...
export interface ImportCounts {
  ...
  hospitalGroups: number;
  hospitalSites: number;
  hospitalSitesUpdated: number;
  deployments: number;
  deploymentCategories: number;
...
export async function runImportPayload(client: PoolClient, body: AnyRec): Promise<ImportResult> {
